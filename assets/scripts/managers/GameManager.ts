...
export class GameManager extends Component {
    ...
    private gameOpponentQuantity: number = null;
    private gameOpponentQuantityMaximum: number = 4;

    /** Variable for number of opponents currently active on game scene. */
    private gameOpponentCountActive: number = 0;

    /** Function to create the opponents. */
    private instantiateOpponents(): void {
        // Code to add opponents to the node pool.
        let gameOpponentNode = null;
        for (let index = 1; index < this.gameOpponentQuantityMaximum; index++) {
            gameOpponentNode = instantiate(this.gameOpponentPrefab);
            this.gameOpponentNodePool.put(gameOpponentNode);
        }
    }
    ...
    private addOpponent(): void {
        ...
        if (this.gameOpponentNodePool.size() > 0) {
            gameOpponentNode = this.gameOpponentNodePool.get();
        }
        else {
            gameOpponentNode = instantiate(this.gameOpponentPrefab);
            this.gameOpponentNodePool.put(gameOpponentNode);
        }
        gameOpponentNode.setPosition(this.gameOpponentInstancesNode.getPosition().x + 100 , 0, this.gameOpponentInstancesNode.getPosition().z);
        this.gameOpponentInstancesNode.addChild(gameOpponentNode);
        gameOpponentNode.getComponent(OpponentController).activateOpponent(gameOpponentAnimation, this.gameDifficulty);
    }

    /** Function to remove opponents from scene. */
    private removeOpponent(value: Node): void {
        this.gameOpponentNodePool.put(value);
    }

    /** Function to manage the opponents based on  */
    public manageOpponents(value: boolean, node?: Node): void {
        if (value == false) {
            this.removeOpponent(node);
            this.gameOpponentCountActive--;
        }
        while (this.gameOpponentCountActive < this.gameOpponentQuantity) {
            this.addOpponent();
            this.gameOpponentCountActive++;
        }
    }
    ...
    public initialiseGame(): void {
        ...
        this.changeGameDifficulty(GAME_DIFFICULTY.HARD);
    ...
    /** Function to change the game's difficulty. */
    private changeGameDifficulty(value: GAME_DIFFICULTY): void {
